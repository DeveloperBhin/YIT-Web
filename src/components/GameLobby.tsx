...
interface TokenPayload {
  ...
  email: string;
  phone_number: string;
  token?: string; // keep token for socket events
}
...
export default function GameLobby({
  ...
  const handleCreateRoom = () => {
    ...
    setError('');
    socket.emit('create_game', { maxPlayers, token: user.token });
  };
  ...
  const handleJoinRoom = () => {
    ...
    setError('');
    socket.emit('join_game', { gameId: roomCode, token: user.token });
  };
    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen text-white bg-[#0B1E4F] overflow-hidden">
      <Particles
        ...
        options={{
          ...
          fpsLimit: 60,
          interactivity: { events: { onHover: { enable: true, mode: 'repulse' }, resize: { enable: true } }, modes: { repulse: { distance: 100, duration: 0.4 } } },
          particles: { color: { value: '#ffffff' }, links: { enable: true, distance: 150, color: '#ffffff', opacity: 0.3, width: 1 }, move: { enable: true, speed: 1, outModes: { default: 'bounce' } }, number: { value: 60, density: { enable: true } }, opacity: { value: 0.3 }, shape: { type: 'circle' }, size: { value: { min: 1, max: 4 } } },
          detectRetina: true,
        }}
      ...
      >
        ...
        <input type="text" readOnly className="p-3 rounded w-64 text-black" value={user ? `${user.first_name} ${user.second_name}` : ''} />

        <input type="number" min={2} max={10} className="p-3 rounded w-64 text-black" value={maxPlayers} onChange={(e) => setMaxPlayers(Number(e.target.value))} />

        <div className="flex gap-4">
          <button onClick={handleCreateRoom} className="bg-green-500 px-6 py-2 rounded-lg font-semibold hover:bg-green-600 transition">Create Game</button>
          <input type="text" placeholder="Room code" className="p-3 rounded w-48 text-black" value={roomCode} onChange={(e) => setRoomCode(e.target.value)} />
          <button onClick={handleJoinRoom} className="bg-yellow-500 px-6 py-2 rounded-lg font-semibold hover:bg-yellow-600 transition">Join Game</button>
        </div>

        <div className="w-full max-w-md mt-6">
          <h3 className="text-lg font-semibold mb-2 text-center text-yellow-300">Available Games (last 2 min)</h3>
          {availableRooms.length === 0 ? <p className="text-gray-300 text-center">No active game yet — create one!</p> : (
            <ul className="space-y-2">
              {availableRooms.map((room) => (
                <motion.li key={room.gameId} className="bg-blue-600 p-3 rounded-lg flex justify-between items-center"
                  initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}
                >
                  <div>
                  ...
                  <button onClick={() => setRoomCode(room.gameId)} className="bg-white text-blue-800 px-3 py-1 rounded hover:bg-gray-200 transition">Join</button>
                </motion.li>
              ))}
            </ul>
          )}
